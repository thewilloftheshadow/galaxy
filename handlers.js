...
module.exports = client => {
   fs.readdir("./commands/", (err, files) => {
    files.forEach(file => {
        ...
        fs.readdir(path, (err, files) => {
          ...
          if(re.config.debug) console.debug(`\nLoading ${jsfile.length} commands in the ${file} category...`);
          jsfile.forEach((f, i) =>{
            let props = require(`./commands/${file}/${f}`);
            if(re.config.debug) console.debug(`${i + 1}: Command "${f.split(".js").join("")}" loaded`)
            try{
              ...
              if(props.help.alias){
                  ...
                  props.help.alias.forEach(alias => {
                    ...
                    if(re.config.debug) console.debug(` Alias ${alias} added`);
                  });
              }
            }
            catch(err){
  ...
  fs.readdir("./events/", (err, files) => {
    ...
    if(re.config.debug) console.debug(`Loading ${jsfiles.length} events...`);
    jsfiles.forEach((f, i) => {
        require(`./events/${f}`);
        if(re.config.debug) console.debug(`${i + 1}: Event "${f.split(".js").join("")}" loaded`);
    });
});
}