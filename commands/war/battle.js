...
module.exports.run = async (client, message, args) => {
  ...
  user2.send(new re.Discord.MessageEmbed().setAuthor(message.author.tag, message.author.avatarURL()).setDescription(`[${message.author.username} has challenged you to a battle!](https://discord.com/channels/${message.guild.id}/${message.channel.id}/{m.id} "Click here to accept the challenge!")`)).catch(e=>{})
  m.edit(`${user2}, ${message.member.displayName} has challenged you to a battle! Do you accept?`)
  ...
  let reactions = await m
      ...
      if (reaction.id != "678023486618468363"){ return await m.edit("Battle declined!"); 
  await m.reactions.removeAll()
